...
# ################### WORKING CODE A #########################################################
# This code works with VerseSelecto.tsx tagged as working code A #############################
...
#         text = data.get("text")
#         language = data.get("language", "en")
#         pause_duration = float(data.get("pause_duration", 0.0))  # Default to 0 seconds if not provided
...
#         # Convert text to SSML with pauses between words
#         words = text.split()
#         ssml_text = "<speak>"
#         for i, word in enumerate(words):
#             ssml_text += word
#             if i < len(words) - 1:  # Add pause before the last word
#                 ssml_text += f"<break time='{pause_duration}s'/>"
#         ssml_text += "</speak>"
...
#         print("❌ TTS ERROR: Invalid pause_duration value:", ve)
#         return jsonify({"error": "Invalid pause_duration value"}), 400
#     except Exception as e:
#         print("❌ TTS ERROR:", e)
#         return jsonify({"error": str(e)}), 500
...
def tts():
    ...
    try:
        ...
        text = data.get("text")
        language = data.get("language", "en")
        pause_duration = float(data.get("pause_duration", 0.0))  # Default to 0 seconds

        if not text:
            return jsonify({"error": "Missing 'text' in request"}), 400

        # Cap pause_duration to avoid extreme values that disrupt speech
        pause_duration = min(pause_duration, 1.0)  # Limit to 1 second maximum
